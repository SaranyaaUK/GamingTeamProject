...
public class AIScoring {
    ...
    public static Map<Tile, Unit> getBestActionForUnit(Unit unit) {
        ...
        for (Tile movableTile : movableTiles) {
            ...
            List<Tile> adjacentEnemyTiles = TilesGenerator.getEnemiesWithinAttackDistanceForAI(movableTile);
    ...
    public static double getScoreForPosition(Unit unit, Tile position) {
        ...
        score += getScoreForProximityToAvatars(unit, gameState.getHumanPlayer().getAvatar(), gameState.getAIPlayer().getAvatar());
    ...
    private static double getBountyForFlyingPosition(Tile position) {
        List<Tile> adjacentEnemyTiles = TilesGenerator.getEnemiesWithinAttackDistanceForAI(position);
        if (adjacentEnemyTiles.isEmpty()) {
    ...
    private static double getScoreForProximityToAvatars(Unit unit, Unit humanAvatar, Unit AIAvatar) {
        double score = 0;
        if (unit.isHumanUnit()) {
            score += getDistanceBetweenUnit(unit, humanAvatar) * getBountyfForDistanceFromMyGeneral(humanAvatar.getHealth());
            score += getDistanceBetweenUnit(unit, AIAvatar) * BOUNTY.DISTANCE_FROM_OPPONENT_GENERAL;
        } else {
            score += getDistanceBetweenUnit(unit, AIAvatar) * getBountyfForDistanceFromMyGeneral(AIAvatar.getHealth());
            score += getDistanceBetweenUnit(unit, humanAvatar) * BOUNTY.DISTANCE_FROM_OPPONENT_GENERAL;
        }
        return score;
    }
    ...
    public static double getDamageScore(Unit targetUnit, int ATK) {
        ...
        if (remainingHealth <= 0) {
            ...
            ArrayList<Unit> humanUnits = gameState.getHumanPlayer().getMyUnits();
    ...
    private static int getDistanceBetweenUnit(Unit unit, Unit target) {
        Tile unitTile = TilesGenerator.getUnitTile(unit);
        Tile targetTile = TilesGenerator.getUnitTile(target);
        return Math.abs(unitTile.getXpos() - targetTile.getXpos()) + Math.abs(unitTile.getYpos() - targetTile.getYpos());
    }
    ...
    public static double getDeathWatchScore(Unit unit) {
        ...
        if (unit instanceof ShadowDancer) {
            score += getDamageScore(GameState.getInstance().getAIPlayer().getAvatar(), 1);
